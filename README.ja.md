...
このインストーラーは、サーバーとロガーの統合管理を提供する `vigil` アプリケーションのセットアップと設定を自動化します。

### 機能

*   GitHub Releases から最新の `vigil` および `vigil-logger` バイナリを自動的にダウンロードします。
*   システム起動時に `vigil` が自動的に開始されるように設定します（Linux では systemd、macOS では launchd、Windows ではスタートアップフォルダを使用）。
...
`config.toml` を変更した後、設定を反映させるには **Vigil サービスを再起動**する必要があります。お使いのオペレーティングシステムに合わせて、以下のいずれかの方法を選択してください。
...
*   **macOS:**
    ...
    *   **手動:** コンピュータを再起動します。
*   **Windows:**
    *   **コマンド (PowerShell):** `Stop-Process -Name "vigil" -Force; start-process "$env:APPDATA\Microsoft\Windows\Start Menu\Programs\Startup\vigil.exe"`
    *   **手動:** コンピュータを再起動するか、タスクマネージャーで `vigil.exe` を見つけてプロセスを終了し、スタートアップフォルダから再度実行してください。
